...
// // // Engine RPM PCNT configuration
// // const pcnt_config_t pcnt_cfg_engine {
// //     .pulse_gpio_num = PIN_ENGINE_RPM,
// //     .ctrl_gpio_num = PCNT_PIN_NOT_USED,
// //     .lctrl_mode = PCNT_MODE_KEEP,
// //     .hctrl_mode = PCNT_MODE_KEEP,
// //     .pos_mode = PCNT_COUNT_INC,
// //     .neg_mode = PCNT_COUNT_DIS,  // Only count rising edges for engine RPM
// //     .counter_h_lim = PCNT_H_LIM,
// //     .counter_l_lim = 0,
// //     .unit = PCNT_ENGINE_RPM,
// //     .channel = PCNT_CHANNEL_0
// // };
...
// // static void IRAM_ATTR on_pcnt_overflow_engine(void* args) {
// //     portENTER_CRITICAL_ISR(&engine_mux);
// //     engine_overflow++;
// //     portEXIT_CRITICAL_ISR(&engine_mux);
// // }
...
//     CHECK_ESP_FUNC(gpio_set_direction(PIN_ENGINE_RPM, GPIO_MODE_INPUT), "Failed to set PIN_ENGINE_RPM to Input! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(gpio_set_pull_mode(PIN_ENGINE_RPM, GPIO_PULLUP_ONLY), "Failed to set PIN_ENGINE_RPM to Input! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_unit_config(&pcnt_cfg_engine), "Failed to configure PCNT for Engine RPM reading! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_counter_pause(PCNT_ENGINE_RPM), "Failed to pause PCNT Engine RPM! %s", esp_err_to_name(res))    // Clear their stored values (If present)
...
//     CHECK_ESP_FUNC(pcnt_counter_clear(PCNT_ENGINE_RPM), "Failed to clear PCNT Engine RPM! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_set_filter_value(PCNT_ENGINE_RPM, 1000), "Failed to set filter for PCNT Engine! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_filter_enable(PCNT_ENGINE_RPM), "Failed to enable filter for PCNT Engine! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_isr_handler_add(PCNT_ENGINE_RPM, &on_pcnt_overflow_engine, nullptr), "Failed to add PCNT Engine to ISR handler! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_event_enable(PCNT_ENGINE_RPM, pcnt_evt_type_t::PCNT_EVT_H_LIM), "Failed to register event for PCNT Engine! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(pcnt_counter_resume(PCNT_ENGINE_RPM), "Failed to resume PCNT Engine RPM! %s", esp_err_to_name(res))
...
//     CHECK_ESP_FUNC(timer_isr_register(TIMER_GROUP_0, TIMER_0, &RPM_TIMER_ISR, NULL, 0, &rpm_timer_handle), "%s", esp_err_to_name(res))    CHECK_ESP_FUNC(timer_start(TIMER_GROUP_0, TIMER_0), "%s", esp_err_to_name(res))
    
//     // Setup PWM for RPM gauge output
//     ledcSetup(RPM_GAUGE_CHANNEL, 1000, 8); // Start with 1kHz, 8-bit resolution
//     ledcAttachPin(PIN_RPM_GAUGE_OUT, RPM_GAUGE_CHANNEL);
//     ledcWrite(RPM_GAUGE_CHANNEL, 0); // Start with no output
